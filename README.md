...
6. `Space`를 처음 누르면 1280px H.264 스티칭 프록시를 한 번 생성하고, 완료 후에는
   Space로 즉시 재생/일시정지합니다. 프록시는 같은 소스·TC·IN/OUT·설정 조합에서 재사용됩니다.
7. 현재 타임라인만 바로 출력하려면 `RENDER NOW`, 여러 테이크를 모아 순차 출력하려면
...
고정 투영 맵을 모든 프레임에 재사용합니다. Space 플레이백 역시 같은 맵을 사용하지만
원본을 약 1280px로 디코드하고 optical flow를 끈 8-bit H.264 검수 프록시이므로 최종
10/12-bit 마스터 품질에는 영향을 주지 않습니다. 처음 생성할 때만 시간이 들고 이후에는
일반 플레이어처럼 재생·정지·스크럽합니다.
